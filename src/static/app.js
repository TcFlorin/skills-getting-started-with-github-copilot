document.addEventListener("DOMContentLoaded", () => {
        ...
        const participantsMarkup = participants.length > 0
          ? `<ul class="participants-list">${participants.map((participant) => `
              <li class="participant-item">
                ...
                <button type="button" class="participant-delete" data-activity="${name}" data-email="${participant}" aria-label="Remove ${participant} from ${name}">✕</button>
              </li>`).join("")}</ul>`
  ...
  function activityListDeleteHandlers() {
